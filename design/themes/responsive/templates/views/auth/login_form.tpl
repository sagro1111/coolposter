...
{hook name="index:login_buttons"}
	...
	<div class="ty-float-right">{include file="buttons/login.tpl" but_name="dispatch[auth.login]" but_role="submit"}</div>
	
	<div class="ty-login__remember-me ty-newsletters__item">
	<label for="remember_me_{$id}" class="ty-login__remember-me-label"><input id="remember_me_{$id}" type="checkbox" name="remember_me" value="Y" class="checkbox">{__("remember_me")}</label>
	</div>
