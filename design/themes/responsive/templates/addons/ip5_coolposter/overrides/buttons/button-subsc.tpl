...
{if $but_name && $but_role != "text" && $but_role != "act" && $but_role != "delete"} {* SUBMIT BUTTON *}
    <button {if $but_id}id="{$but_id}"{/if} class="{$but_meta} ty-btn-go ty-search-magnifier" type="submit" name="{$but_name}" {if $but_onclick}onclick="{$but_onclick nofilter}"{/if}><span class="sub-ico"></span></button>
