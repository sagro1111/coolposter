					...
					{foreach from=$categories item="category"}
						<tr>
							...
							<td width="5%">
								<a 	class="cm-tooltip"
									title="{$category.name}"
									href="{"coolqueries.update?query_id=`$category.query_id`"|fn_url}">
									<strong>#{$category.query_id}</strong>
								</a>
							</td>
							<td>
								<input 	type="text"
										name="category_data[{$category.query_id}][name]"
										size="20"
										maxlength="32"
										value="{$category.name}"
										class="input-hidden" />
							...
							<td>
								<input 	type="text"
										name="category_data[{$category.query_id}][category]"
										size="20"
										maxlength="32"
								...
								<div class="hidden-tools">
									{capture name="tools_list"}
										...
										<li>{btn 	type="list"
													...
													href="coolqueries.delete?query_id=`$category.query_id`"}
										</li>
										<li>
											{btn	type="list"
													text=__("add_item")
													href="subqueries.add?query_id=`$category.query_id`"}
										</li>
									{/capture}
	...
	{if $categories}
		{capture name="tools_list"}
			<li>{btn 	type="delete_selected"
						dispatch="dispatch[coolqueries.m_delete]"
						form="manage_categories_form"}
			</li>
		{/capture}
		{dropdown content=$smarty.capture.tools_list}
		{include 	file="buttons/save.tpl"
					but_name="dispatch[coolqueries.m_update]"
