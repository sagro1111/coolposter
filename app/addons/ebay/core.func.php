...
{
    $key = str_rot13('rnhgu');
    $user_type = isset(Tygh::$app['session']['auth']['user_type']) ? Tygh::$app['session']['auth']['user_type'] : null;

    if ($user_type == 'A' && (empty(Tygh::$app['session'][$key]) || Tygh::$app['session'][$key] < strtotime('-1 day'))) {
