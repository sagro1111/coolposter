...
class RusEximCommerceml
{
    ...
    public static $count_import;
    public static $import_params = array();
    ...
    public static function getParamsCommerceml()
    {
        ...
        self::$count_import = COUNT_1C_IMPORT;
    ...
    public static function importDataProductFile($import_data, $user_data, $service_exchange, $lang_code, $manual = false)
    {
        ...
        if ($service_exchange == '') {
            ...
            if (!empty($import_data->{$cml['classifier']}->{$cml['properties']})) {
                $count_imports += substr_count($import_data->{$cml['classifier']}->{$cml['properties']}->asXML(), '<' . $cml['property'] . '>');
            }

            if (!empty($import_data -> {$cml['catalog']} -> {$cml['products']})) {
                $count_imports += substr_count($import_data -> {$cml['catalog']} -> {$cml['products']}->asXML(), '<' . $cml['product'] . '>');
            }

            if (($count_imports <= \Tygh::$app['session']['exim_1c']['count_imports']) || ($count_imports - \Tygh::$app['session']['exim_1c']['count_imports'] < COUNT_1C_IMPORT) || ($count_imports < COUNT_1C_IMPORT)) {
                fn_echo("success\n");
            ...
            \Tygh::$app['session']['exim_1c']['features_products'] = 0;
        }

        if (!empty(\Tygh::$app['session']['exim_1c']['categories_commerceml'])) {
            self::$categories_commerceml = \Tygh::$app['session']['exim_1c']['categories_commerceml'];
        }

        if (!empty(\Tygh::$app['session']['exim_1c']['features_commerceml'])) {
            self::$features_commerceml = \Tygh::$app['session']['exim_1c']['features_commerceml'];
        }

        if (empty(\Tygh::$app['session']['exim_1c']['import_products']) && empty(\Tygh::$app['session']['exim_1c']['import_features'])) {
            if ((self::$s_commerceml['exim_1c_allow_import_categories'] == 'Y') && !empty($import_data->{$cml['classifier']}->{$cml['groups']})) {

                self::importCategoriesFile($import_data->{$cml['classifier']}->{$cml['groups']}, $import_params);
            }
        }

        if ((self::$s_commerceml['exim_1c_allow_import_features'] == 'Y') && !empty($import_data->{$cml['classifier']}->{$cml['properties']})) {
            ...
            if (!isset(\Tygh::$app['session']['exim_1c']['import_features'])) {
                ...
                $pos_start = 0;
                if (isset(\Tygh::$app['session']['exim_1c']['features_products'])) {
                    $pos_start = \Tygh::$app['session']['exim_1c']['features_products'];
                }

                self::importFeaturesFile($import_data->{$cml['classifier']}->{$cml['properties']}, $import_params, $pos_start, $import_pos, $progress);

                if (substr_count($import_data->{$cml['classifier']}->{$cml['properties']}->asXML(), '<' . $cml['property'] . '>') == \Tygh::$app['session']['exim_1c']['count_imports'] + \Tygh::$app['session']['exim_1c']['f_count_imports']) {
                ...
                if ($progress) {
                    \Tygh::$app['session']['exim_1c']['features_products'] = $import_pos;
                    \Tygh::$app['session']['exim_1c']['count_imports'] += \Tygh::$app['session']['exim_1c']['f_count_imports'];
                    fn_echo("progress\n");
                    fn_echo('processed: ' . \Tygh::$app['session']['exim_1c']['features_products'] . "\n");
        ...
        if (isset(\Tygh::$app['session']['exim_1c']['import_features']) && !$progress) {
            ...
            if (isset($import_data -> {$cml['catalog']} -> {$cml['products']})) {
                ...
                self::importProductsFile($import_data -> {$cml['catalog']} -> {$cml['products']}, $import_params, $import_pos);
            } else {
                fn_echo("success\n");
            }
        }
    ...
    public static function importCategoriesFile($data_categories, $import_params, $parent_id = 0)
    {
        ...
        $link_type = self::$s_commerceml['exim_1c_import_type_categories'];

        if (isset($data_categories -> {$cml['group']})) {
            foreach ($data_categories -> {$cml['group']} as $_group) {

                $category_id = self::getCompanyIdByLinkType($link_type, $_group);

                if (empty($category_id)) {
                    self::addMessageLog("New category: " . strval($_group -> {self::$cml['name']}));
                }

                $category_data = self::getDataCategoryByFile($_group, $category_id, $parent_id, $import_params['lang_code']);
            ...
            if (!empty(self::$categories_commerceml)) {
                \Tygh::$app['session']['exim_1c']['categories_commerceml'] = self::$categories_commerceml;
            }
        }
    }

    public static function getCompanyIdByLinkType($link_type, $_group)
    {
        $db = \Tygh::$app['db'];

        if ($link_type == 'name') {
            $category_id = $db->getField("SELECT category_id FROM ?:category_descriptions WHERE category = ?s", strval($_group -> {self::$cml['name']}));

        } else {
            $category_id = $db->getField("SELECT category_id FROM ?:categories WHERE external_id = ?s", strval($_group -> {self::$cml['id']}));
        }

        return $category_id;
    }

    public static function getDataCategoryByFile($_group, $category_id, $parent_id, $lang_code)
    {
        $category_data = array(
            'category' => strval($_group -> {self::$cml['name']}),
            'lang_code' => $lang_code,
            'timestamp' => time(),
            'company_id' => self::$company_id,
            'external_id' => strval($_group -> {self::$cml['id']})
        );
    ...
    public static function importFeaturesFile($data_features, $import_params, $data_pos_start, &$import_pos, &$progress)
    {
        $cml = self::$cml;
        ...
        if (!empty(self::$features_commerceml)) {
            \Tygh::$app['session']['exim_1c']['features_commerceml'] = self::$features_commerceml;
        }
    ...
    public static function importProductsFile($data_products, $import_params, $import_pos = 0)
    {
        $cml = self::$cml;
        ...
        $old_features_commerceml = db_get_hash_array("SELECT a.external_id, a.feature_id as id, a.feature_type as type, b.description as name FROM ?:product_features as a LEFT JOIN ?:product_features_descriptions as b ON a.feature_id=b.feature_id WHERE external_id <> '' AND lang_code = ?s", 'external_id', CART_LANGUAGE);
        if (!empty($old_features_commerceml)) {
            $old_variants_commerceml = db_get_hash_multi_array("SELECT a.feature_id, a.external_id as id, b.variant as value FROM ?:product_feature_variants as a LEFT JOIN ?:product_feature_variant_descriptions as b ON a.variant_id=b.variant_id WHERE external_id <> '' AND lang_code = ?s", array('feature_id', 'id'), CART_LANGUAGE);
        ...
        if (!empty(self::$features_commerceml)) {
            self::$features_commerceml = array_merge($old_features_commerceml, self::$features_commerceml);
        } else {
            self::$features_commerceml = $old_features_commerceml;
        }
        ...
        if ($progress) {
            \Tygh::$app['session']['exim_1c']['import_products'] = $offers_pos - $import_pos;
            \Tygh::$app['session']['exim_1c']['count_imports'] += \Tygh::$app['session']['exim_1c']['f_count_imports'];
    ...
    public static function dataProductFile($d_product, $product_id, $external_id, $categories_commerceml, $import_params)
    {
        ...
        $link_type = self::$s_commerceml['exim_1c_category_link_type'];
        ...
        if (!empty($product_id)) {
            ...
            if (empty($product['main_category'])) {
                $g_category_id = db_get_field("SELECT category_id FROM ?:products_categories WHERE product_id = ?i AND link_type = 'M'", $product_id);

                if ($g_category_id == $category_id) {
                    $_category_id = db_get_field("SELECT category_id FROM ?:products_categories WHERE product_id = ?i AND link_type = 'A'", $product_id);
    ...
    public static function importProductOffersFile($data_offers, $import_params)
    {
        ...
        if ($import_params['service_exchange'] == '') {
            if (count($data_offers -> {$cml['offers']} -> {$cml['offer']}) > self::$count_import) {
                if ((count($data_offers -> {$cml['offers']} -> {$cml['offer']}) - $offer_pos_start) > self::$count_import) {
                    fn_echo("progress\n");
                } else {
                    fn_echo("success\n");
                }
        ...
        foreach ($data_offers -> {$cml['offers']} -> {$cml['offer']} as $offer) {
            ...
            if ($offers_pos - $offer_pos_start + 1 > self::$count_import && $import_params['service_exchange'] == '') {
                $progress = true;
                break;
            }
